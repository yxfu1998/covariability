...
def _padded_xy_limits_from_scatter(x_vals, y_vals, pad_frac=0.14):
    """Add relative padding to scatter min/max to reduce marker clipping at edges."""
    ...
    dx = x1 - x0
    dy = y1 - y0
    pad_x = max(dx * pad_frac, 1e-15) if dx > 0 else max(abs(x0) * 0.02 + 1e-15, 1e-12)
    pad_y = max(dy * pad_frac, 1e-15) if dy > 0 else max(abs(y0) * 0.02 + 1e-15, 1e-12)
    return x0 - pad_x, x1 + pad_x, y0 - pad_y, y1 + pad_y


def _padded_1d_limits(vals, pad_frac=0.08):
    """Add padding to 1D min/max (e.g. KDE x-axis)."""
    ...
    d = v1 - v0
    p = max(d * pad_frac, 1e-15) if d > 0 else max(abs(v0) * 0.02 + 1e-15, 1e-12)
    return v0 - p, v1 + p
...
def draw_combined_scatter_plot(obs_dir, cmip6_dir, output_path):
    ...
    print("Processing row 1 col 1: ERA5 PDF (198001-202412/202412)")
    ...
    for i, (var_x, var_y, label) in enumerate(var_combos):
        ...
        # Periods: rows 2,3,5 and others per config
        # i=0: Row1row (TCWV vs SST) - 198001-202412
        # i=1: Row2row (TCWV vs TMT) - 198001-202412
        # i=2: Row3row (TCWV vs TLT) - 198001-202412
        # i=3: Row4row (TMT vs TLT) - 198001-202412
        # i=4: Row5row (TMT vs SST) - 198001-202412
        if i in [1, 2, 3]:
            start_yyyymm = 198001
            end_yyyymm = 202412
        else:
    ...
    col2_configs = [
        ('merra2', 'merra2', 'sst', 'cwv', 198001, 202412, 'MERRA2 TCWV vs MERRA2 SST'),   # Row 1, same as fig3
        ('merra2', 'merra2', 'tmt', 'cwv', 198001, 202412, 'MERRA2 TCWV vs MERRA2 TMT'),
        ('merra2', 'merra2', 'tlt', 'cwv', 198001, 202412, 'MERRA2 TCWV vs MERRA2 TLT'),
        ('merra2', 'merra2', 'sst', 'tmt', 198001, 202412, 'MERRA2 TMT vs MERRA2 SST'),
    ]
    ...
    for i, (var_x, var_y, label) in enumerate(var_combos):
        ...
        slopes_finite = np.asarray(slopes, dtype=float)
        slopes_finite = slopes_finite[np.isfinite(slopes_finite)]
        if slopes_finite.size == 0:
            continue

        if len(obs_slopes) > 0:
            obs_arr = np.asarray(obs_slopes, dtype=float)
            obs_arr = obs_arr[np.isfinite(obs_arr)]
            xs_union = np.concatenate([slopes_finite, obs_arr]) if obs_arr.size > 0 else slopes_finite
        else:
            xs_union = slopes_finite

        lim_x = _padded_1d_limits(xs_union, pad_frac=0.10)
        if lim_x is None:
            continue
        x_min, x_max = lim_x
        # Rows 2-3: x minimum fixed at 0(and consistent with supply-style plots)
        if i in (1, 2):
            x_min = 0.0
            x_max = np.max(xs_union)
            d = x_max - x_min
            p = max(d * 0.1, 1e-15)
            x_max = x_max + p


        # KDE/quantiles on CMIP6 slopes within x limits
        ...
        # Set x-axis limits(from CMIP6+obs slope union with padding)
        ...
        # Pad y by KDE max so curve and markers fit
        if y_kde_max > 0:
            ax_kde.set_ylim(0.0, y_kde_max * 1.18)
        else:
    ...
    plt.subplots_adjust(left=0.18, right=0.93, top=0.98, bottom=0.08, wspace=0.1, hspace=0.23)
...
def main():
    ...
    # Output path
    output_path = os.path.join(out_dir, "fig4.png")
