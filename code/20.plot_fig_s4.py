...
def main():
    """
    Plot TLT anomaly differences and trends for all dataset pairs.
    ...
    # --- Read each dataset  TLT anomaly,time 1980–2024 ---
    ...
    for filepath in data_files:
        ...
        time_sel = time[mask]
        tlt_sel = variables["tlt"][mask]

        # use 13.plot_fig1 in  calculate_monthly_anomaly algorithm
        anomaly = calculate_monthly_anomaly(tlt_sel)

        # TLT anomaly no percent conversion; use K
        label = get_dataset_label(filepath)
        tlt_dict[label] = (time_sel.astype(int), anomaly.astype(float))

    # TLTno USTC; use STAR as reference
    if "STAR" not in tlt_dict:
        print("not found STAR   TLT data，Cannot compute differences.")
        return
    ...
    def compute_pair_metrics(label1, label2, start_yyyymm_seg, end_yyyymm_seg):
        ...
        if (label1 not in tlt_dict) or (label2 not in tlt_dict):
            return None
        time1, anom1 = tlt_dict[label1]
        time2, anom2 = tlt_dict[label2]
    ...
    all_labels = sorted(tlt_dict.keys())
    print(f"find {len(all_labels)}  TLT datasets: {all_labels}")

    # Col 1: STAR minus others (no USTC TLT); from 1980
    star_results = []
    rss_results = []
    uah_results = []
    
    if "STAR" in all_labels:
        ref_time, ref_anom = tlt_dict["STAR"]
    ...
    if "RSS" in all_labels:
        ref_time, ref_anom = tlt_dict["RSS"]
        start_yyyymm = 198001
        end_yyyymm = 202412
        ref_mask = (ref_time >= start_yyyymm) & (ref_time <= end_yyyymm)
        ref_time_sub = ref_time[ref_mask]
    ...
    fig.text(0.04, 0.5, "TLT Anomaly Difference (K)", va="center", rotation="vertical", fontsize=10, weight='bold')

    # adjustlayout
    fig.tight_layout(rect=[0.05, 0.05, 0.95, 0.95])
    plt.subplots_adjust(wspace=0.15, hspace=0.3)
    out_name = "../plot/fig_s4.png"
